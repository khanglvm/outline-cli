# Outline CLI reference

[Back to the README](../README.md)

## Search and documents

Search across documents or ask a question:
...
List available tools and their arguments:
...
Use `memory.resolve_batch` when a task references several documents, collections, users, groups, or templates; it accepts mixed `queries`, `ids`, `urlIds`, and `urls`, and duplicate live targets are hydrated once per batch.

Use `memory.recent` as a zero-network history view and `memory.remember` for explicit local aliases.

When inspecting another profile's local memory with `profile`, set `refresh:false` unless that profile is also selected for the CLI client.

Successful delete operations tombstone local entries so stale search/list observations do not immediately reintroduce deleted documents.
